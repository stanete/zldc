...
public class AlertsDataTask extends TimerTask {
    ...
    public void run() {
        ...
        try{
            ...
            //烟雾故障
            addOrRemove(null,"Smoke sensing fault",Alerts,faultState[0],false);
            ...
            for(int i=0;i<subDatas.size();i++){
                ...
                LocalData.ChargeData chargeData = LocalData.getChargeData(String.valueOf(i+1));
                ...
                addOrRemove(i+1,"Sub control fault",Error,runState[2],false);
                //充电器故障
                LocalData.alertChargerOTH[i]=runState[4];
                addChargeAlert(i+1,ci,false);
                //电池锁故障
                LocalData.alertBatLock[i]=runState[5];
                addOrRemove(i+1,"Battery lock failure",Error,runState[5],false);
            }
        }catch(Exception e){
    ...
    private void addBmsAlert(Integer slotId,LocalData.BmsData bd,boolean remove){
        ...
        addOrRemove(slotId,"pack over voltage warning",Error,chargInFault[9],remove);
