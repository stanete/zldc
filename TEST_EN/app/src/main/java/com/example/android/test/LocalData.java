...
public class LocalData {
    ...
    public class BmsData {
        ...
        public BmsData setData(byte[] data) {
            ...
            //充电机控制开关
            bd.setContrSwitch((data[off++]&0xff)==0);
            //控制工作模式
...
//            bd.setCommonFault((data[off++] & 0xFF) + ((data[off++] << 8) & 0xFF00));
     ...
     * @return
     */
    public static LocalData.SubData getSubData(int boxNum) {
    ...
    public class HashCheck{
        ...
        public void sendHash(Integer sH){
            if(sendHash.size()>Integer.MAX_VALUE){
        ...
        public void revicvedHash(Integer rH){
            if(revicvedHash.size()>Integer.MAX_VALUE){
                throw new RuntimeException();
            }
    ...
    public static Version getVersion(){
        if(version==null){
            version=new LocalData().new Version();
        }
        return version;
    }
    ...
    public class Version{
        ...
        public Version(String mainVer, String subVer) {
    ...
    public static boolean[] isBatOnline(){
        boolean[] result=new boolean[8];
        try{
            for(int i=0;i<subDataList.size();i++){
                result[i]=subDataList.get(i).getRuningState()[0];
            }
        }catch(Exception e){
